...
#?(:clj
   (defn -main [& args]
     (let [seed-path (or (first args) "kotoba/seed.edn")
           actor-root (or (System/getenv "ATSURAE_REPO")
                          (System/getProperty "user.dir"))
           log-path (or (second args)
                        (str (clojure.java.io/file actor-root "data" "persisted"
                                                  "atsurae.line.kotoba.edn")))
